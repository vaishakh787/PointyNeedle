...
# Consistent device/session IDs
USER_ID = uuid.uuid4()
DEVICE_ID = uuid.uuid4()

# SRP config for Apple compatibility
srp.rfc5054_enable()
srp.no_username_in_x()

ANISETTE_URL = "http://127.0.0.1:6969"  # anisette-v3-server
...
def gsa_authenticate(username, password, second_factor="sms"):
    ...
    if r["sp"] not in ["s2k", "s2k_fo"]:
        raise Exception(f"Unsupported SRP protocol {r['sp']}")

    usr.p = encrypt_password(password, r["s"], r["i"], r["sp"])
    M = usr.process_challenge(r["s"], r["B"])
    if M is None:
        raise Exception("Failed to process SRP challenge")

    r = gsa_authenticated_request({"c": r["c"], "M1": M, "u": username, "o": "complete"})
    if "M2" not in r:
        raise Exception("Missing M2 in server response")

    usr.verify_session(r["M2"])
    if not usr.authenticated():
        raise Exception("SRP session verification failed")

    spd = decrypt_cbc(usr, r["spd"])
    spd = plist.loads(b"""<?xml version='1.0' encoding='UTF-8'?>
<!DOCTYPE plist PUBLIC '-//Apple//DTD PLIST 1.0//EN' 
'http://www.apple.com/DTDs/PropertyList-1.0.dtd'>""" + spd)

    if "au" in r.get("Status", {}):
        au = r["Status"]["au"]
        if au in ["trustedDeviceSecondaryAuth", "secondaryAuth"]:
            ...
            raise Exception(f"Unknown auth flow {au}")
...
def generate_meta_headers(serial="0", user_id=uuid.uuid4(), device_id=uuid.uuid4()):
    ...
    return {
        "X-Apple-I-Client-Time": now_utc.isoformat().replace("+00:00", "Z"),
        "X-Apple-I-TimeZone": str(now_utc.astimezone().tzinfo),
        "loc": locale.getdefaultlocale()[0] or "en_US",
        "X-Apple-Locale": locale.getdefaultlocale()[0] or "en_US",
...
def decrypt_cbc(usr, data):
    ...
    try:
        ...
        raise ValueError("Invalid PKCS7 padding bytes during CBC decryption") from e
...
def trusted_second_factor(dsid, idms_token):
    ...
    requests.get("https://gsa.apple.com/auth/verify/trusteddevice", headers=headers, verify=False, timeout=10)

    code = SECOND_FACTOR_CALLBACK() if callable(SECOND_FACTOR_CALLBACK) else getpass("Enter 2FA code: ")
    headers["security-code"] = code

    resp = requests.get("https://gsa.apple.com/grandslam/GsService2/validate", headers=headers, verify=False, timeout=10)
    if resp.ok:
        print("2FA successful")
...
def sms_second_factor(dsid, idms_token):
    ...
    body = {"phoneNumber": {"id": 1}, "mode": "sms"}
    requests.put("https://gsa.apple.com/auth/verify/phone/", json=body, headers=headers, verify=False, timeout=5)

    code = SECOND_FACTOR_CALLBACK() if callable(SECOND_FACTOR_CALLBACK) else input("Enter 2FA code: ")
    body["securityCode"] = {"code": code}

    resp = requests.post("https://gsa.apple.com/auth/verify/phone/securitycode", json=body, headers=headers, verify=False, timeout=5)
    if resp.ok:
        print("2FA successful")
